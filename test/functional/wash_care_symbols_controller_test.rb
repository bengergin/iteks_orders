...
class WashCareSymbolsControllerTest < ActionController::TestCase
  def setup
    @controller = WashCareSymbolsController.new
  ...
  def test_create_without_user
    assert_no_difference 'WashCareSymbol.count' do
      post :create, {}
      assert_redirected_to new_session_path
    end
  end
  ...
  def test_create_blank_wash_care_symbol_with_user
    assert_no_difference 'WashCareSymbol.count' do
